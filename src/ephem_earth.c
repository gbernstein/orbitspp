/* 	$Id: ephem_earth.c,v 2.1 2002/01/20 22:09:09 garyb Exp $	 */
#ifndef lint
static char vcid[] = "$Id: ephem_earth.c,v 2.1 2002/01/20 22:09:09 garyb Exp $";
#endif /* lint */
		  ...
		  double *vxyz)
{
  ...
  if (body==MOON) {
    /* For Moon position need to combine E-M and EM barycenter*/
    double embary[3], vembary[3];
    Interpolate_State(jd, EARTH, embary, vembary);
    for (i=0; i<3; i++) {
      posn[i] = embary[i] + posn[i]*R1.EMRAT/(1.+R1.EMRAT);
    }
    if (vxyz!=NULL)
      for (i=0; i<3; i++) {
	vel[i] = vembary[i] + vel[i]*R1.EMRAT/(1.+R1.EMRAT);
      }
  }
