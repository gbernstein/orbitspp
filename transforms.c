/* 	$Id: transforms.c,v 2.2 2002/02/13 17:56:07 garyb Exp $	 */

#ifndef lint
static char vcid[] = "$Id: transforms.c,v 2.2 2002/02/13 17:56:07 garyb Exp $";
#endif /* lint */
...
/*Conversions between equatorial coordinates and invariable-plane
 *coordinates.  Longitude in the latter is defined to start at
 *ascending node on equator.
 */
void
eq_to_inv(double ra,
	  double dec,
	  ...
	  double *lon)
{
  double rr;
  double	sd,cd,cr,se,ce,y,x;
  rr = ra - INVARIABLE_NODE;

  se = sin(INVARIABLE_INCL);
  ce = cos(INVARIABLE_INCL);

  sd = ce * sin(dec) - se * cos(dec)*sin(rr);
  *lat = asin(sd);

  y = ce*cos(dec)*sin(rr) + se*sin(dec);
  x = cos(dec)*cos(rr);
  *lon = atan2(y,x);
}
void
inv_to_eq(double lat,
	  double lon,
	  double *ra,
	  double *dec)
{
  ...
  sd = ce * sin(lat) - se * cos(lat)*sin(lon);
  *dec = asin(sd);

  y = ce*cos(lat)*sin(lon) + se*sin(lat);
  x = cos(lat)*cos(lon);
  *ra = atan2(y,x) + INVARIABLE_NODE;
}
